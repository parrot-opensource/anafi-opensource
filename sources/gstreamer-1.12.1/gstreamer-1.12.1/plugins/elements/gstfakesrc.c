...
gst_fake_src_data_get_type (void)
{
  ...
  static const GEnumValue fakesrc_data[] = {
    ...
    {FAKE_SRC_DATA_PATTERN_BUFFER, "Data from a buffer", "pattern-buffer"},
    {0, NULL, NULL},
  };
...
*/
  ...
  /**
   * GstFakeSrc:pattern-buffer
   *
   * Set the a #GstBuffer as a filling pattern
   */
  g_object_class_install_property (gobject_class, PROP_PATTERN_BUFFER,
      g_param_spec_boxed ("pattern-buffer", "Pattern Buffer",
          "GstBuffer that contains the pattern", GST_TYPE_BUFFER,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_fake_src_finalize (GObject * object)
{
  ...
  if (src->pattern_buffer)
    gst_buffer_unref (src->pattern_buffer);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_FORMAT:
      ...
      break;
    case PROP_PATTERN_BUFFER:
      gst_buffer_replace (&src->pattern_buffer, g_value_get_boxed (value));
      break;
    default:
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_FORMAT:
      ...
      break;
    case PROP_PATTERN_BUFFER:
      g_value_set_boxed (value, src->pattern_buffer);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gst_fake_src_get_size (GstFakeSrc * src)
{
  ...
  /* when data is 'pattern-buffer' use the buffer size */
  if (src->data == FAKE_SRC_DATA_PATTERN_BUFFER && src->pattern_buffer)
    return gst_buffer_get_size (src->pattern_buffer);
...
gst_fake_src_create_buffer (GstFakeSrc * src, gsize * bufsize)
{
  ...
  *bufsize = size;
  ...
  switch (src->data) {
    case FAKE_SRC_DATA_PATTERN_BUFFER:
      if (src->pattern_buffer == NULL)
        goto buffer_create_fail;
      buf = gst_buffer_copy (src->pattern_buffer);
      break;
...
gst_fake_src_start (GstBaseSrc * basesrc)
{
  ...
  if (src->data == FAKE_SRC_DATA_PATTERN_BUFFER
      && src->pattern_buffer == NULL) {
    GST_WARNING_OBJECT (src, "src data set to 'pattern-buffer' without"
       " 'pattern-buffer' property set");
    return FALSE;
  }
