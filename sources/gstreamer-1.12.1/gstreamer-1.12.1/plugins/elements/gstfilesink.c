...
#define DEFAULT_POST_CHECK FALSE
#define DEFAULT_FREE_SPACE_LIMIT 0
...
gst_file_sink_class_init (GstFileSinkClass * klass)
{
  ...
  /**
   * GstFileSink::post-check
   *
   * Check buffer integrity after the write and report GST_FLOW_ERROR
   * on corrupted buffer
   */
  g_object_class_install_property (gobject_class, PROP_POST_CHECK,
      g_param_spec_boolean ("post-check", "Post Check",
          "Check buffer integrity after the write", DEFAULT_POST_CHECK,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /**
   * GstFileSink::free-space-limit
   * Check remaining free space before write and report GST_FLOW_ERROR
   * NO_SPACE_LEFT if free space not available
   */
  g_object_class_install_property (gobject_class, PROP_FREE_SPACE_LIMIT,
      g_param_spec_uint64 ("free-space-limit", "Free space limit",
          "Check free space limit available before write", 0, G_MAXUINT64,
          DEFAULT_FREE_SPACE_LIMIT,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_file_sink_init (GstFileSink * filesink)
{
  ...
  filesink->post_check = FALSE;
  filesink->free_space_limit = DEFAULT_FREE_SPACE_LIMIT;
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_APPEND:
      ...
      break;
    case PROP_POST_CHECK:
      sink->post_check = g_value_get_boolean (value);
      break;
    case PROP_FREE_SPACE_LIMIT:
      sink->free_space_limit = g_value_get_uint64 (value);
      break;
    default:
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_APPEND:
      ...
      break;
    case PROP_POST_CHECK:
      g_value_set_boolean (value, sink->post_check);
      break;
    case PROP_FREE_SPACE_LIMIT:
      g_value_set_uint64 (value, sink->free_space_limit);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static gint
gst_file_sink_check_free_space_limit (GstFileSink * sink, GstBuffer ** buffers, guint num_buffers)
{
  gint i, ret;
  guint64 free_space, free_space_needed;
  struct statvfs vstat;

  /* bypass free space limit if required */
  if (sink->free_space_limit == 0) {
      return GST_FLOW_OK;
  }

  /* compute free space needed
   * If a buffer has the NON_DROPPABLE flag, skip check */
  free_space_needed = sink->free_space_limit;
  for (i = 0; i < num_buffers; i++) {
    free_space_needed += gst_buffer_get_size(buffers[i]);
    if (GST_BUFFER_FLAG_IS_SET (buffers[i], GST_BUFFER_FLAG_NON_DROPPABLE)) {
      GST_DEBUG_OBJECT (sink, "Buffer is NON_DROPPABLE, skip free space check");
      return GST_FLOW_OK;
    }
  }

  /* get remaining space */
  ret = statvfs(sink->filename, &vstat);
  if (ret < 0)
     return GST_FLOW_ERROR;

  free_space = vstat.f_bfree * vstat.f_bsize;

  /* check remaining space */
  if (free_space < free_space_needed) {
     GST_WARNING_OBJECT (sink,
      "Free space limit %" G_GUINT64_FORMAT " reached", sink->free_space_limit);
    ...
    guint num_buffers, guint8 * mem_nums, guint total_mems)
{
  ...
  /* Check free space */
  ret = gst_file_sink_check_free_space_limit (sink, buffers, num_buffers);
  ...
  if (G_UNLIKELY (sink->post_check && ret == GST_FLOW_OK)) {
    guint i;
    for (i = 0; i < num_buffers; i++) {
      if (gst_buffer_check (buffers[i]) == FALSE) {
        GST_ELEMENT_ERROR (sink, STREAM, FORMAT, (_("Error at buffer check")),
            (NULL));
        return GST_FLOW_ERROR;
      }
    }
