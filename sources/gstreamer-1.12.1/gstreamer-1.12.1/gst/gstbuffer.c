...
typedef struct _GstBufferCheckerItem GstBufferCheckerItem;

struct _GstBufferCheckerItem
{
  GstBufferChecker checker;
  gpointer data;
};

static G_DEFINE_QUARK (gst-buffer-checker, gst_buffer_checker)
#define GST_BUFFER_CHECKER_QUARK gst_buffer_checker_quark ()

void
gst_buffer_set_checker (GstBuffer * buffer, GstBufferChecker checker, gpointer
    data)
{
  GstBufferCheckerItem *bcheck;

  bcheck = g_malloc (sizeof (GstBufferCheckerItem));
  bcheck->checker = checker;
  bcheck->data = data;

  gst_mini_object_set_qdata (GST_MINI_OBJECT (buffer), GST_BUFFER_CHECKER_QUARK,
      bcheck, g_free);
}

gboolean
gst_buffer_check (GstBuffer * buffer)
{
  GstBufferCheckerItem *bcheck;

  bcheck = gst_mini_object_get_qdata (GST_MINI_OBJECT (buffer),
      GST_BUFFER_CHECKER_QUARK);

  if (bcheck && bcheck->checker)
    return bcheck->checker (buffer, bcheck->data);
