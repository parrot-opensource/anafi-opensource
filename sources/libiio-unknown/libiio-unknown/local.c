		...
		const char *attr, const char *src, size_t len, bool is_debug)
{
	...
	/* Open buffer/enable fd only once */
	if (!strncmp(attr, "buffer/enable", sizeof("buffer/enable"))) {
		if (!dev->pdata->buffer_enable)
			dev->pdata->buffer_enable = fopen(buf, "we");
		f = dev->pdata->buffer_enable;
	}
	else
		f = fopen(buf, "we");
...
static int local_close(const struct iio_device *dev)
{
	...
	if (pdata->buffer_enable) {
		fclose(pdata->buffer_enable);
		pdata->buffer_enable = NULL;
	}
...
static int local_get_timeout(struct iio_context *ctx, unsigned int *timeout)
{
	*timeout = ctx->pdata->rw_timeout_ms;
	return 0;
}
