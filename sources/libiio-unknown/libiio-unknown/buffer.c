...
ssize_t iio_buffer_flush(struct iio_buffer *buffer)
{
	int ret = -EINVAL;
	ssize_t nsamples = 0;
	bool blocking = iio_buffer_get_blocking_mode(buffer);

	ret = iio_buffer_set_blocking_mode(buffer, false);
	if (ret < 0)
		return ret;
	...
	ret = iio_buffer_set_blocking_mode(buffer, blocking);
	if (ret < 0)
		return ret;
	else
		return nsamples;
...
err_refill:
	iio_buffer_set_blocking_mode(buffer, blocking);
	return ret;
}
...
ssize_t iio_buffer_refill(struct iio_buffer *buffer)
{
	...
	if (read >= 0) {
		...
		/* only update sample size if channel mask changed */
		if (dev->mask_changed || buffer->sample_size == 0) {
