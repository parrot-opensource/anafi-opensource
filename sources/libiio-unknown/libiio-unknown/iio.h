...
/** @brief Get current timeout for I/O operations
 * @param ctx A pointer to an iio_context structure
 * @param timeout_ms A pointer to a positive integer representing the time in
 * milliseconds after which a timeout occurs. A value of 0 is used to specify that no
 * timeout should occur.
 * @return On success, 0 is returned
 * @return On error, a negative errno code is returned */
__api int iio_context_get_timeout(
		struct iio_context *ctx, unsigned int *timeout_ms);
...
/** @brief Fetch and drop all already available samples in a buffer
 *
 * This fetches samples in a buffer using non-blocking mode, so
 * only backends that support non-blocking mode are supported.
 * @param buf A pointer to an iio_buffer structure
 * @return On success, the number of samples flushed is returned
 * @return On error, a negative errno code is returned
 */
__api ssize_t iio_buffer_flush(struct iio_buffer *buf);
