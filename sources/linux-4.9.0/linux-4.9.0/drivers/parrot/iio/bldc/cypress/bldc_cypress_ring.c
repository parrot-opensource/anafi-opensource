...
irqreturn_t bldc_cypress_read_fifo(int irq, void *p)
{
	...
	result = st->tf->read_multiple_byte(st->dev,
			...
			PARROT_BLDC_OBS_DATA_LENGTH, rx, 1);
	...
	for (i = 0; i < indio_dev->num_channels - 1; i++, chan++) {
		if (!test_bit(chan->scan_index, indio_dev->buffer->scan_mask)) {
			if (chan->scan_type.storagebits == 24) {
				rx++;
				rx++;
			}
		...
		if (chan->scan_type.storagebits == 24) {
			__be32 data = *((__be32 *)rx);
			*tx++ = data & 0xff;
			*tx++ = (data >> 8) & 0xff;
			*tx++ = (data >> 16) & 0xff;
			rx += 3;
		} else if (chan->scan_type.storagebits == 16) {
