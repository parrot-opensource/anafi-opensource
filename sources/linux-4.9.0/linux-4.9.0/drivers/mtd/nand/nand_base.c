				...
				u8 id_data[8], int *busw)
{
	...
	if (id_len == 6 && id_data[0] == NAND_MFR_SAMSUNG &&
		...
		*busw = (extid & 0x01) ? NAND_BUSWIDTH_16 : 0;
		...
		if (id_len >=5 && id_data[0] == NAND_MFR_TOSHIBA) {
			/*
			 ...
			 */
			if (id_len >= 6 && nand_is_slc(chip) &&
					(id_data[5] & 0x7) == 0x6 /* 24nm */ &&
					!(id_data[4] & 0x80) /* !BENAND */) {
				mtd->oobsize = 32 * mtd->writesize >> 9;
			}

			if (mtd_nand_has_benand() && id_data[4] & 0x80) {
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	case NAND_ECC_BENAND:
		...
		if (ecc->read_page)
			pr_warn("ECC read_page overrided to support benand\n");
		ecc->read_page = nand_read_page_benand;

		if (ecc->read_subpage)
			pr_warn("ECC read_subpage overrided to support benand\n");
