				...
				struct iio_chan_spec const *ch, int *val)
{
	...
	if (indio_dev->currentmode == INDIO_BUFFER_TRIGGERED) {
		...
		for_each_set_bit(cid, indio_dev->active_scan_mask,
			...
			sb = chan->scan_type.storagebits / 8;
		...
		if (chan_found) {
			byte_for_channel = ch->scan_type.realbits >> 3;

			if (byte_for_channel == 1)
				*val = (s8)*buf;
			else if (byte_for_channel == 2)
				*val = (s16)get_unaligned_le16(buf);
			else if (byte_for_channel == 3)
				*val = (s32)st_sensors_get_unaligned_le24(buf);
			...
			const struct st_sensor_settings *sensor_settings)
{
	...
	for (i = 0; i < num_sensors_list; i++) {
		for (n = 0; n < ST_SENSORS_MAX_4WAI; n++) {
			...
			/*
			 * To keep compat with standard drivers, previous loop
			 * is kept as is.
			 * However, it prevents from having multiple devices
			 * with the same driver. A drv_name member is added in
			 * st_sensor_data to identify the name of the driver and
			 * to retrieve it correctly in the current context.
			 */
			if (strcmp(sdata->drv_name,
				sensor_settings[i].sensors_supported[n]) == 0) {
				break;
			}
		}
