...
/* Enable this flag to force the payloader to collect all rtp packet of
 * one access unit in a buffer list befor pushing them to downstream element.
 * This method rely on the "end_of_au" flag that seems to only work with AU
 * alignment mode.
 *
 * The purpose of this is to lower the downstream processing (typically udpsink)
 * by allowing it to used vectored syscall (like sendmmsg)
 * The drawback is that we intracte the latency as we wait for the full AU which
 * is not mandatory in byte-stream foramt.
 */
#define PUSH_AU_NAL_ONCE
...
#ifdef PUSH_AU_NAL_ONCE
    if (rtph264pay->alignment == GST_H264_ALIGNMENT_AU) {
      if (rtph264pay->au_outbuf_list == NULL)
        rtph264pay->au_outbuf_list = gst_buffer_list_new ();

      gst_buffer_list_add (rtph264pay->au_outbuf_list, outbuf);

      if (end_of_au) {
        ret = gst_rtp_base_payload_push_list (basepayload,
            rtph264pay->au_outbuf_list);
        rtph264pay->au_outbuf_list = NULL;
      } else {
        ret = GST_FLOW_OK;
      }
    } else {
      /* simply push the buffer to the next element */
