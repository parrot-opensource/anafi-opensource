...
#define AK8975_CONVERSION_DONE_POLL_TIME_AVG	7
#define AK8975_CONVERSION_DONE_POLL_TIME_MAX	9
...
static int ak8975_soft_reset(struct i2c_client *client)
{
	...
	if (ret < 0) {
		dev_err(&client->dev, "Error in writing CNTL2 : retrying\n");
		ret = i2c_smbus_write_byte_data(data->client, AK8963_REG_CNTL2,
					AK8963_REG_CNTL2_SRST_RESET);
	}
	if (ret < 0) {
...
static int wait_conversion_complete_gpio(struct ak8975_data *data)
{
	...
	while (timeout_ms) {
		usleep_range(AK8975_CONVERSION_DONE_POLL_TIME_AVG * 1000,
				AK8975_CONVERSION_DONE_POLL_TIME_MAX * 1000);
		if (gpio_get_value(data->eoc_gpio))
			break;
		/* approximate to minimum sleep time */
		timeout_ms -= AK8975_CONVERSION_DONE_POLL_TIME_AVG;
	}
	if (!timeout_ms) {
			...
			const struct i2c_device_id *id)
{
	...
	indio_dev->name = client->dev.of_node ?
