...
static ssize_t show_ctrlrequest(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	ret = kfifo_get(&udc->ctrlreq_fifo, &ctrl);
	if (ret == 0) {
		/* fifo empty */
	...
	return sprintf(buf, "%02x.%02x v%04x i%04x l%d\n",
		ctrl.bRequestType, ctrl.bRequest,
		w_value, w_index, w_length);
}

static DEVICE_ATTR(ctrlrequest, S_IRUSR, show_ctrlrequest, NULL);

static int create_sysfs_files(struct ambarella_udc *udc)
{
	int ret;

	ret = sysfs_create_file(&udc->dev->kobj, &dev_attr_ctrlrequest.attr);
	if (ret) {
		dev_err(udc->dev, "device_create_file failed\n");
		return ret;
	}
...
static void remove_sysfs_files(struct ambarella_udc *udc)
{
	sysfs_remove_file(&udc->dev->kobj, &dev_attr_ctrlrequest.attr);
}
...
static void ambarella_disable_all_intr(struct ambarella_udc *udc)
{
	...
	writel_relaxed(0x000000ff, udc->base_reg + USB_DEV_INTR_MSK_REG);
	writel_relaxed(0xffffffff, udc->base_reg + USB_DEV_EP_INTR_MSK_REG);
	writel_relaxed(0x000000ff, udc->base_reg + USB_DEV_INTR_REG);
...
static struct ambarella_data_desc *ambarella_get_last_desc(struct ambarella_ep *ep)
{
	...
	if (desc && !(desc->status & USB_DMA_LAST))
		pr_debug("last desc stat=%x(%d) last=%d\n", desc->status, desc->status & USB_DMA_LAST,desc->last_aux);
	return desc;
}
...
static void ambarella_handle_request_packet(struct ambarella_udc *udc)
{
	...
	if (ret < 0) {
		/* notify control request failed in user space */
		if (kfifo_is_full(&udc->ctrlreq_fifo)) {
			pr_err("fifo is full, drop oldest request\n");
			kfifo_skip(&udc->ctrlreq_fifo);
		}

		ret = kfifo_put(&udc->ctrlreq_fifo, *crq);
		if (ret != 0) {
			sysfs_notify(&udc->dev->kobj, NULL, "ctrlrequest");
		} else {
			pr_err("kfifo_put error %d\n", ret);
		}
...
static int ambarella_handle_data_out(struct ambarella_ep *ep)
{
	...
	if (req->req.actual)
		pr_debug("out actual %d\n", req->req.actual);
	req_size = req->req.length - req->req.actual;
	if (recv_size > req_size) {
		pr_debug("recv_size > req_size %d > %d (%d)\n", recv_size, req_size, ep->ep.maxpacket);
		if ((req_size % ep->ep.maxpacket) != 0) {
			pr_err("recv_size > req_size %d > %d (%d)\n", recv_size, req_size, ep->ep.maxpacket);
...
static void udc_device_interrupt(struct ambarella_udc *udc, u32 int_value)
{
	...
	/* case 2. Get reset Interrupt */
		...
		if (!udc->host_suspended && udc->driver) {
			spin_unlock(&udc->lock);
	...
	/* case 3. Get suspend Interrupt */
		...
		pr_err("%s: USB suspend IRQ 0x%x\n", __func__,
				readl_relaxed(udc->base_reg + USB_DEV_INTR_MSK_REG));
...
static void udc_epin_interrupt(struct ambarella_udc *udc, u32 ep_id)
{
	...
	if (ambarella_check_bna_error(ep, ep_status)
		...
		ep->need_cnak = 0;
		pr_err("%s: ep is bna/he error 0x%08x\n", ep->ep.name, ep_status);
		if (!list_empty(&ep->queue)) {
			req = list_first_entry(&ep->queue, struct ambarella_request,queue);
			req->req.status = -EPROTO;
			ambarella_udc_done(ep, req, 0);
		...
		pr_err("%s: ep is error 0x%08x (empty list)\n", ep->ep.name, ep_status);
		return;
	}
...
static irqreturn_t ambarella_udc_irq(int irq, void *_dev)
{
	...
	if(value) {
		...
		pr_debug("device int value = 0x%x\n", value);
		...
		gfp_t gfp_flags)
{
	...
	val = readl_relaxed(udc->base_reg + USB_DEV_STS_REG);
	if(val & USB_DEV_SUSP_STS){
		pr_err("%s: UDC is suspended! 0x%x\n", __func__, val);
		return -ESHUTDOWN;
	}
...
static int ambarella_udc_probe(struct platform_device *pdev)
{
	...
	INIT_KFIFO(udc->ctrlreq_fifo);
	create_sysfs_files(udc);
