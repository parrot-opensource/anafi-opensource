...
static int uvc_video_decode_start(struct uvc_streaming *stream,
	struct uvc_buffer *buf, const __u8 *data, int len, u64 delay_ns)
{
	__u8 fid;
	...
	if (buf->state != UVC_BUF_STATE_ACTIVE) {
		...
		buf->buf.vb2_buf.timestamp = timespec_to_ns(&ts) - delay_ns;
...
static void uvc_video_decode_isoc(struct urb *urb, struct uvc_streaming *stream,
	struct uvc_buffer *buf)
{
	u64 interval_ns, delay_ns;
	u8 *mem;
	int ret, i;

	/* Calculate delay of isochronous packets from top of list */
	if (urb->dev->speed <= USB_SPEED_FULL)
		interval_ns = urb->interval * 1000000ULL;
	else
		interval_ns = urb->interval * 125000ULL;
	delay_ns = urb->number_of_packets * interval_ns;
	...
	for (i = 0; i < urb->number_of_packets; ++i) {
		...
		do {
			ret = uvc_video_decode_start(stream, buf, mem,
				urb->iso_frame_desc[i].actual_length, delay_ns);
			if (ret == -EAGAIN) {
	...
	struct uvc_buffer *buf)
{
	...
	if (stream->bulk.header_size == 0 && !stream->bulk.skip_payload) {
		do {
			ret = uvc_video_decode_start(stream, buf, mem, len, 0);
			if (ret == -EAGAIN)
				buf = uvc_queue_next_buffer(&stream->queue,
							    buf);
		} while (ret == -EAGAIN);
