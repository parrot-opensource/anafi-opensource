...
static irqreturn_t ambarella_i2c_irq(int irqno, void *dev_id)
{
	...
	case AMBA_I2C_STATE_START:
		if (ambarella_i2c_check_ack(pinfo, &control_reg,
			...
			} else if (pinfo->msgs->flags & I2C_M_RD) {
				if (pinfo->msgs->len == 1)
	...
	int num)
{
	...
	down(&pinfo->system_event_sem);
		...
		errorCode = 0;
		if (pinfo->state != AMBA_I2C_STATE_IDLE) {
			dev_err(pinfo->dev, "not in idle %d, reseting...\n", pinfo->state);
		...
		if (timeout <= 0) {
			pinfo->state = AMBA_I2C_STATE_TIMEOUT;
		}
		...
		else if (pinfo->state != AMBA_I2C_STATE_IDLE)
			errorCode = -EIO;

	up(&pinfo->system_event_sem);

	if (errorCode) {
		if (errorCode == -EREMOTEIO) {
			dev_dbg(pinfo->dev,
