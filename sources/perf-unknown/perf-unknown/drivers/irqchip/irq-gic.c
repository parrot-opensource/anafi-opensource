...
static void gic_irq_set_target(struct irq_data *d)
{
	...
	mask = 0xff << shift;
	bit = gic->saved_spi_target[gic_irq(d) >> 2] & mask;
...
static void gic_enable_irq(struct irq_data *d)
{
	...
	/* Some irq (nand) are share between linux and threadx
	   restore target
	 */
			    ...
			    bool force)
{
	...
	/* save the mask for the enable : for shared irq between
	   linux and threadx (nand) we need to reset the mask
	   in enable
	 */
	gic->saved_spi_target[gic_irq(d) >> 2] = val | bit;
#endif
	writel_relaxed(val | bit, reg);
	gic_unlock_irqrestore(flags);
