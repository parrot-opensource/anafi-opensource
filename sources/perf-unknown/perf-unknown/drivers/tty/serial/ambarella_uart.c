...
static void serial_ambarella_hw_setup(struct uart_port *port)
{
	...
	writel_relaxed(UART_FIFO_SIZE-5, port->membase + UART_RX_TH_OFFSET);
	...
	if (amb_port->txdma_used || amb_port->rxdma_used) {
		...
		*/
		...
		writel_relaxed(0xe, port->membase + UART_RX_TH_OFFSET);
	...
	u32 tmo)
{
	...
	do {
		...
		if (unlikely(ls & (UART_LS_BI | UART_LS_PE |
			...
			if (ls & UART_LS_OE) {
				printk(KERN_DEBUG "ttyS%d: uart OVERFLOW\n",
					port->line);
			}
		}
...
#ifdef CONFIG_OF
static void ambarella_console_early_putchar(struct uart_port *port, int ch)
{
	if (!port->suspended) {
		wait_for_tx(port);
		udelay(10);
...
static void ambarella_console_early_write(struct console *con, const char *s,
				    unsigned count)
{
	struct earlycon_device *dev = con->data;

	uart_console_write(&dev->port, s, count, ambarella_console_early_putchar);
}

static int __init
ambarella_console_early_setup(struct earlycon_device *dev, const char *opt)
{
	u32 lcr;
	if (!dev->port.membase)
		return -ENODEV;

	/* We depend of threadx uart1 init */
	lcr = readl_relaxed(dev->port.membase + UART_LC_OFFSET);
	if ((lcr & 3) != UART_LC_CLS_8_BITS) {
		pr_err("uart1 is not enabled by threadx.\n");
		return -ENODEV;
	}

	dev->con->write = ambarella_console_early_write;

	return 0;
}
OF_EARLYCON_DECLARE(ec_amba, "ambarella,uart", ambarella_console_early_setup);
#endif
