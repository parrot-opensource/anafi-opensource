...
struct spit_file {
	...
	/* Frame types mapped and started */
...
static int spit_close(struct inode *inode, struct file *filp)
{
	...
	atomic_dec(&spit_devdata->opened);

	mutex_lock(&spit_devdata->lock);
	if (spit_file->started) {
		/* Stop all current frames stream on device */
		dev_warn(spit_devdata->dev, "stream not stopped: kill stream\n");
		rpmsg.type = SPIT_RPMSG_STREAM_TYPE_REQ_KILL;
		rpmsg.stream = spit_devdata->stream;
		rpmsg.frame_types = spit_file->started;
		rpmsg_send(rp_device->ept, &rpmsg, sizeof(rpmsg));

		/* Update started status */
		spit_devdata->started &= ~spit_file->started;
	}
	mutex_unlock(&spit_devdata->lock);
...
static void spit_flush_buffers(struct spit_file *spit_file)
{
	dev_info(spit_file->spit_devdata->dev, "%d old buffers flushed\n",
...
static inline void spit_sync_cache(struct spit_devdata *spit_devdata,
				   struct spit_fifo_map *map, void *paddr,
				   size_t size, int flush)
{
	long overflow;
	...
	/* Flush / invalidate buffer area L1 and L2 caches */
	if (flush)
		dma_sync_single_for_device(spit_devdata->dev,
					   (dma_addr_t) paddr, size - overflow,
					   DMA_TO_DEVICE);
	else
	...
	/* FIFO doesn't support truncated frames */
	...
	/* Flush / invalidate tail of frame in head buffer */
	if (flush)
		dma_sync_single_for_device(spit_devdata->dev,
					   (dma_addr_t) map->paddr, overflow,
					   DMA_TO_DEVICE);
	else
		       ...
		       unsigned long arg)
{
	...
	case SPIT_FEED_FRAME:
	{
		...
		/* Flush cache for frame */
		spit_sync_cache(spit_devdata, map, map->paddr + offset,
				frame_desc.size, 1);
	...
	case SPIT_GET_FRAME:
	{
		...
		} while (!SPIT_FRAME_TYPES_HAS_TYPE(spit_file->started,
						    frame_desc.type));
	...
	case SPIT_SET_INPUT_CACHE:
	{
		...
		if (cache.type >= SPIT_FRAME_TYPE_COUNT)
			return -EINVAL;

		if (req == SPIT_SET_INPUT_CACHE)
			cache.type += SPIT_FRAME_TYPE_COUNT;
