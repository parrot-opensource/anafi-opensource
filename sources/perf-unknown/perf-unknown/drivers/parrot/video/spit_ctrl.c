...
#define SPIT_CTRL_WAIT_TIMEOUT (2*HZ)
			      ...
			      void *priv, u32 src)
{
	...
	} else if (msg->type == SPIT_RPMSG_CONTROL_TYPE_EVT_STATUS) {
		/* Copy new control status */
		spit_ctrl_devdata->status = msg->status;
		atomic_set(&spit_ctrl_devdata->status_updated, 1);
			    ...
			    unsigned long arg)
{
	...
	case SPIT_CTRL_GET_STATUS:
		/* Copy status */
		if (copy_to_user((void __user *)arg, &spit_ctrl_devdata->status,
				 sizeof(enum spit_control_status))) {
			return -EFAULT;
		}
		break;
...
static unsigned int spit_ctrl_poll(struct file *filp, poll_table *wait)
{
	...
	poll_wait(filp, &spit_ctrl_devdata->ctrl_wait, wait);

	/* Check if status has been updated */
	if (atomic_read(&spit_ctrl_devdata->status_updated)) {
		atomic_set(&spit_ctrl_devdata->status_updated, 0);
