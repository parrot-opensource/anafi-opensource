...
static gchar *
float_to_fraction_string (gfloat num)
{
  gint frac_n;
  gint frac_d;
...
static gchar *
serialize_tiff_chroma_positioning (const GValue * value)
{
  const gchar *str;
  gint num;
  ...
  str = g_value_get_string (value);
  if (str == NULL) {
    GST_WARNING ("Failed to get YCbCrPositioning tag value");
    return NULL;
  }
  num = __exif_tag_image_chroma_positioning_to_exif_value (str);
  if (num == -1)
    return NULL;
...
static void
deserialize_tiff_chroma_positioning (XmpTag * xmptag, GstTagList * taglist,
    const gchar * gst_tag, const gchar * xmp_tag, const gchar * str,
    GSList ** pending_tags)
{
    guint value;
    const gchar *chroma_positioning = NULL;

    if (sscanf (str, "%u", &value) != 1) {
      GST_WARNING ("Failed to parse tiff:YCbCrPositioning %s", str);
      return;
    }
    ...
    chroma_positioning =
        __exif_tag_image_chroma_positioning_from_exif_value (value);
    if (chroma_positioning == NULL)
      return;
    gst_tag_list_add (taglist, xmp_tag_get_merge_mode (xmptag), gst_tag,
        chroma_positioning, NULL);
}

inline static gchar *
serialize_angle (const GValue * value)
{
  gfloat angle;
  angle = g_value_get_float (value);
...
_init_xmp_tag_map (gpointer user_data)
{
  ...
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_IMAGE_CHROMA_POSITIONING,
      "tiff:YCbCrPositioning", GstXmpTagTypeSimple,
      serialize_tiff_chroma_positioning, deserialize_tiff_chroma_positioning);
  ...
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_DRONE_ROLL,
      "drone-parrot:DroneRollDegree", GstXmpTagTypeSimple, serialize_angle,
      deserialize_angle);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_DRONE_PITCH,
      "drone-parrot:DronePitchDegree", GstXmpTagTypeSimple, serialize_angle,
      deserialize_angle);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_DRONE_YAW,
      "drone-parrot:DroneYawDegree", GstXmpTagTypeSimple, serialize_angle,
      deserialize_angle);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_CAMERA_ROLL,
      "drone-parrot:CameraRollDegree", GstXmpTagTypeSimple, serialize_angle,
      deserialize_angle);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_CAMERA_PITCH,
      "drone-parrot:CameraPitchDegree", GstXmpTagTypeSimple, serialize_angle,
      deserialize_angle);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_CAMERA_YAW,
      "drone-parrot:CameraYawDegree", GstXmpTagTypeSimple, serialize_angle,
      deserialize_angle);

  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_DEVICE_MODEL_ID,
      "drone-parrot:ModelId", GstXmpTagTypeSimple, NULL, NULL);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_DEVICE_SERIAL,
      "drone-parrot:SerialNumber", GstXmpTagTypeSimple, NULL, NULL);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_SOFTWARE_VERSION,
      "drone-parrot:SoftwareVersion", GstXmpTagTypeSimple, NULL, NULL);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_SOFTWARE_BUILD_ID,
      "drone-parrot:SoftwareBuildId", GstXmpTagTypeSimple, NULL, NULL);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_RUN_DATE,
      "drone-parrot:RunDate", GstXmpTagTypeSimple, NULL, NULL);
  _gst_xmp_schema_add_simple_mapping (schema, GST_TAG_RUN_ID,
