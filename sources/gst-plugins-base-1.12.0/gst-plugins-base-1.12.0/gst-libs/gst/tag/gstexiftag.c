...
EXIF_SERIALIZATION_DESERIALIZATION_FUNC (chroma_positioning);
...
EXIF_SERIALIZATION_DESERIALIZATION_FUNC (date_time);
EXIF_SERIALIZATION_DESERIALIZATION_FUNC (color_space);
...
#define EXIF_TAG_CHROMA_POSITIONING 0x213
...
#define EXIF_TAG_OFFSET_TIME_ORIGINAL 0x9011
#define EXIF_TAG_OFFSET_TIME_DIGITALIZED 0x9012
...
static const GstExifTagMatch tag_map_exif[] = {
  ...
  {GST_TAG_DATE_TIME, EXIF_TAG_DATE_TIME_DIGITIZED, EXIF_TYPE_ASCII,
        EXIF_TAG_OFFSET_TIME_DIGITALIZED, serialize_date_time,
      deserialize_date_time},
  {GST_TAG_IMAGE_COMPONENTS_CONFIGURATION, EXIF_TAG_COMPONENTS_CONFIGURATION,
        EXIF_TYPE_UNDEFINED, 0,
      serialize_components_configuration, NULL},
  ...
  {GST_TAG_IMAGE_PIXEL_X_DIMENSION, EXIF_TAG_PIXEL_X_DIMENSION, EXIF_TYPE_SHORT,
      0, NULL, NULL},
  {GST_TAG_IMAGE_PIXEL_Y_DIMENSION, EXIF_TAG_PIXEL_Y_DIMENSION, EXIF_TYPE_SHORT,
      0, NULL, NULL},
  {GST_TAG_CAPTURING_FOCAL_PLANE_X_RESOLUTION,
      EXIF_TAG_FOCAL_PLANE_X_RESOLUTION, EXIF_TYPE_RATIONAL, 0, NULL, NULL},
  {GST_TAG_CAPTURING_FOCAL_PLANE_Y_RESOLUTION,
      EXIF_TAG_FOCAL_PLANE_Y_RESOLUTION, EXIF_TYPE_RATIONAL, 0, NULL, NULL},
  {GST_TAG_CAPTURING_FOCAL_PLANE_RESOLUTION_UNIT,
      EXIF_TAG_FOCAL_PLANE_RESOLUTION_UNIT, EXIF_TYPE_SHORT, 0,
      serialize_focal_plane_resolution_unit,
      deserialize_focal_plane_resolution_unit},
  ...
  {GST_TAG_CAPTURING_WHITE_BALANCE, EXIF_TAG_LIGHT_SOURCE, EXIF_TYPE_SHORT,
...
static void
gst_exif_writer_reset (GstExifWriter * writer)
{
  gst_byte_writer_reset (&writer->tagwriter);
  gst_byte_writer_reset (&writer->datawriter);
}
...
static inline guint
gst_exif_writer_write_get_size (GstExifWriter * writer)
{
  return gst_byte_writer_get_size (&writer->tagwriter) +
      gst_byte_writer_get_size (&writer->datawriter);
}
...
write_exif_ifd (const GstTagList * taglist, guint byte_order,
    guint32 base_offset, guint32 * next_ifd_off,
    const GstExifTagMatch * tag_map)
{
  ...
  for (i = 0; tag_map[i].exif_tag != 0; i++) {
    ...
    if (tag_map[i].gst_tag == NULL) {
      ...
      if (tag_map[i].exif_tag == EXIF_GPS_IFD_TAG) {
        ...
        write_exif_undefined_tag (&writer, EXIF_VERSION_TAG, (guint8 *) "0231",
            4);
  ...
  if (next_ifd_off != NULL) {
    /* the caller explictly ask for a new ifd offset.
       (+sizeof (guint32) is here to take account of this guint32 field) */
    *next_ifd_off = gst_exif_writer_write_get_size (&writer) + sizeof (guint32)
        + base_offset;
    handled &= gst_byte_writer_put_uint32_le (&writer.tagwriter, *next_ifd_off);
  } else {
 ...
 *
 * Returns: A GstBuffer containing the tag entries for the 1st IFD
 */
static GstBuffer *
write_thumb_ifd (const GstTagList * taglist, guint byte_order,
    guint32 base_offset)
{
  GstExifWriter writer;
  GstSample *thumbnail;
  GstBuffer *thumb_buffer;
  guint32 thumb_size, jpeg_interchange_format_pos;
  GstCaps *caps;
  const gchar *mime;
  gdouble hppi, vppi;
  ...
  if (gst_tag_list_get_sample (taglist, GST_TAG_IMAGE, &thumbnail) == FALSE) {
    GST_WARNING ("failed to create the thumbnail");
    return NULL;
  }

  /* check if the thumbnail is a JPEG */
  caps = gst_sample_get_caps (thumbnail);
  mime = gst_structure_get_name (gst_caps_get_structure (caps, 0));
  if (!g_str_equal (mime, "image/jpeg")) {
    GST_WARNING ("not supported thumbnail format: %s", mime);
    return NULL;
  }

  thumb_buffer = gst_sample_get_buffer (thumbnail);
  thumb_size = gst_buffer_get_size (thumb_buffer);

  /* write tag number as 0 */
  gst_byte_writer_put_uint16_le (&writer.tagwriter, 0);

  /* get the horizontal and vertical ppi, 72 as a fallback */
  hppi = vppi = 72;
  gst_tag_list_get_double (taglist, GST_TAG_IMAGE_HORIZONTAL_PPI, &hppi);
  gst_tag_list_get_double (taglist, GST_TAG_IMAGE_VERTICAL_PPI, &vppi);

  /* write mandatory 1st IFD tags */
  gst_exif_writer_write_short_tag (&writer, EXIF_TAG_COMPRESSION, 6); /* JPEG */
  gst_exif_writer_write_rational_tag (&writer, EXIF_TAG_XRESOLUTION, hppi, 1);
  gst_exif_writer_write_rational_tag (&writer, EXIF_TAG_YRESOLUTION, vppi, 1);
  gst_exif_writer_write_short_tag (&writer, EXIF_TAG_RESOLUTION_UNIT, 2);

  jpeg_interchange_format_pos = gst_byte_writer_get_pos (&writer.tagwriter);
  gst_exif_writer_write_long_tag (&writer, EXIF_TAG_JPEG_INTERCHANGE_FORMAT, 0);
  gst_exif_writer_write_long_tag (&writer,
      EXIF_TAG_JPEG_INTERCHANGE_FORMAT_LENGTH, thumb_size);

  /* next ifd offset is 0 */
  ...
  /* rewrite tag offset */
  gst_exif_tag_rewrite_offsets (&writer.tagwriter, byte_order,
      base_offset + gst_byte_writer_get_size (&writer.tagwriter),
      writer.tags_total, NULL);

  /* update the JpegInterchangeFormat */
  gst_byte_writer_set_pos (&writer.tagwriter, jpeg_interchange_format_pos);
  gst_exif_writer_write_long_tag (&writer, EXIF_TAG_JPEG_INTERCHANGE_FORMAT,
      base_offset + gst_exif_writer_write_get_size (&writer));

  /* check if thumbnail will fit in APP1 */
  if (thumb_size + base_offset + gst_exif_writer_write_get_size (&writer)
      + TIFF_HEADER_SIZE > G_MAXUINT16) {
    GST_WARNING ("not enough space to store thumbnail");
    gst_exif_writer_reset (&writer);
    return NULL;
  }

  /* finally copy the thumbnail in the data section */
  gst_byte_writer_put_buffer (&writer.datawriter, thumb_buffer, 0, -1);
...
gst_tag_list_to_exif_buffer (const GstTagList * taglist, gint byte_order,
    guint32 base_offset)
{
  if (gst_tag_list_get_value_index (taglist, GST_TAG_IMAGE, 0)) {
    GstBuffer *ifd0, *ifd1;
    guint32 next_ifd_off;

    ifd0 = write_exif_ifd (taglist, byte_order, base_offset, &next_ifd_off,
        tag_map_ifd0);
    ifd1 = write_thumb_ifd (taglist, byte_order, next_ifd_off);

    if (ifd1)
      return gst_buffer_append (ifd0, ifd1);
    else
      return ifd0;

  } else {
    return write_exif_ifd (taglist, byte_order, base_offset, NULL,
        tag_map_ifd0);
  }
}
...
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC (color_space,
    image_color_space);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC (
    focal_plane_resolution_unit, capturing_focal_plane_resolution_unit);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC (light_source,
    capturing_light_source);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC (chroma_positioning,
...
const GstExifTagMatch * exiftag)
{
  ...
  gchar *off;
  gfloat utc_offset;
  gint utc_offset_hour, utc_offset_min;

  if (!gst_tag_list_get_date_time (taglist, exiftag->gst_tag, &dt)) {
    GST_WARNING ("Failed to GstDateTime from tag list for tag: %s",
        exiftag->gst_tag);
    return;
  }
  ...
  utc_offset_min = (ABS (utc_offset) - utc_offset_hour) * 60;
...
static gint
deserialize_date_time (GstExifReader * exif_reader, GstByteReader * reader,
    const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
...
static void
serialize_components_configuration (GstExifWriter * writer,
    const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
  const gchar *str;
  guint8 data[4], res;
  ...
  if (gst_tag_list_peek_string_index (taglist,
          GST_TAG_IMAGE_COMPONENTS_CONFIGURATION, 0, &str)) {
    if (strlen (str) == 4) {
      for (i = 0; i < 4; i++) {
        res = g_ascii_digit_value (str[i]);
        if (res < 0 || res > 6) {
          ok = FALSE;
          break;
        }
  ...
  if (ok)
    write_exif_undefined_tag (writer, exiftag->exif_tag, data, 4);
  else
    GST_WARNING ("Invalid ComponentsConfiguration \"%s\", "
        "should be a sequence of 4 digits (ranging from 0 to 6). Ignoring it",
        str);
}
